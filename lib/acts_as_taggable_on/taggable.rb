module ActsAsTaggableOn
	module Taggable
		...
		##
		#
		def acts_as_scoped_taggable(tag_type, scope)
			taggable_on_scoped(false, tag_type, scope)
		end

		private

		# same as taggable_on, except expects only one tag type at a time and a scope for that type
		def taggable_on_scoped(preserve_tag_order, tag_type, scope = 'global')
			taggable_on(preserve_tag_order, tag_type)

			if taggable?
				self.tag_types = (self.tag_types + tag_type).uniq
				self.preserve_tag_order = preserve_tag_order
				self.tag_scope[:tag_type] = scope
			else
				class_attribute :tag_scope
				self.tag_scope = {}
				self.tag_scope[:tag_type] = scope
			end
		end
